//! DirectoryManager core: struct, path construction, validation, and CRUD operations.

use std::fs;
use std::path::{Path, PathBuf};

use super::{
    DirectoryError, DirectoryResult, PRODUCTION_DIR, SCRATCH_WORKSTREAM, SESSIONS_DIR, WORK_DIR,
    WORKSTREAMS_DIR,
};
...
impl DirectoryManager {
    ...
    /// # Panics (debug only)
    ///
    /// Debug-asserts that `name` is valid per `is_valid_name()`.
    /// In release builds, invalid names produce paths that won't exist.
    pub fn workstream_path(&self, name: &str) -> PathBuf {
        debug_assert!(
            Self::is_valid_name(name),
            "Invalid workstream name passed to workstream_path: {name:?}"
        );
    ...
    /// # Panics (debug only)
    ///
    /// Debug-asserts that `session_id` is valid.
    pub fn scratch_session_path(&self, session_id: &str) -> PathBuf {
        debug_assert!(
            Self::is_valid_session_id(session_id),
            "Invalid session ID passed to scratch_session_path: {session_id:?}"
        );
    ...
    /// Validate a workstream ID, returning an error if invalid.
    ///
    /// This is the `Result`-returning counterpart to `is_valid_name()`,
    /// intended for use at API boundaries where an error message is needed.
    pub fn validate_workstream_id(id: &str) -> DirectoryResult<()> {
        if Self::is_valid_name(id) {
            Ok(())
        } else {
            Err(DirectoryError::InvalidName(id.to_string()))
        }
    }

    /// Validate a session ID, returning an error if invalid.
    pub fn validate_session_id(id: &str) -> DirectoryResult<()> {
        if Self::is_valid_session_id(id) {
            Ok(())
        } else {
            Err(DirectoryError::InvalidSessionId(id.to_string()))
        }
    }
...
#[cfg(test)]
mod tests {
    use super::super::{DirectoryError, SCRATCH_WORKSTREAM};
    use super::*;
    ...
    #[test]
    fn test_is_valid_name_rejects_traversal() {
        assert!(!DirectoryManager::is_valid_name("../etc"));
        assert!(!DirectoryManager::is_valid_name("../../passwd"));
        assert!(!DirectoryManager::is_valid_name("foo/../bar"));
        assert!(!DirectoryManager::is_valid_name(".."));
    }

    #[test]
    fn test_validate_workstream_id() {
        // Valid IDs
        assert!(DirectoryManager::validate_workstream_id("my-project").is_ok());
        assert!(DirectoryManager::validate_workstream_id("project_123").is_ok());
        assert!(DirectoryManager::validate_workstream_id("scratch").is_ok());

        // Invalid IDs
        assert!(DirectoryManager::validate_workstream_id("").is_err());
        assert!(DirectoryManager::validate_workstream_id("../etc").is_err());
        assert!(DirectoryManager::validate_workstream_id("has/slash").is_err());
        assert!(DirectoryManager::validate_workstream_id("-leading-dash").is_err());
        assert!(DirectoryManager::validate_workstream_id(".hidden").is_err());
    }

    #[test]
    fn test_validate_session_id() {
        assert!(DirectoryManager::validate_session_id("session-abc").is_ok());
        assert!(DirectoryManager::validate_session_id("../escape").is_err());
        assert!(DirectoryManager::validate_session_id("").is_err());
    }
